...
function submitFormEmail(action) {
	var userEmail = document.getElementsByName('mail')[0].value + '@' + document.getElementsByName('slt_mail')[0].value;
	$.ajax({
		type: 'post',
		url: action,
		data: { mail: userEmail },
		success: function(response) {
			if (response.trim() === 'available') {
				document.forms['joinForm'].submit(); // 폼 제출
				//action="MemberJoinCon"
			} else {
