...
public class InquiryViewCon extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        questionsDTO searchTitle = new questionsDTO(null, que_title, null, null, created_at);
        // 3. 회원 아이디 확인
        if (member == null) {
            // 4. 쿠키에서 아이디 가져오기
            ...
            if (cookies != null) {
                for (Cookie cookie : cookies) {
                    if (cookie.getName().equals("savedId")) {
                        ...
                        questionsDTO question = dao.getQuestionByTitle(searchTitle);
                        request.setAttribute("question", question);
                        System.out.println("제목으로 불러온 내용 : " + question);

                        // 6. JSP 페이지로 포워드
                        RequestDispatcher dis = request.getRequestDispatcher("inquiry_view.jsp");
