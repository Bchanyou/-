...
$(document).ready(function() {
	...
	$(".delete").click(function(event) {
		...
		$.ajax({
			type: "POST",
			url: "IngredientSelectDeleteCon", // 서블릿 주소
			data: {
				ingre_idx: ingreIdx
			}, // 서블릿으로 전송할 데이터
		...
		deleteButtons.forEach(function(deleteButton) {
			deleteButton.addEventListener('click',
				function() {
					...
					// 현재 탭에서 선택된 체크박스들 선택
					var checkboxes = this.closest('.tabs_item')
						.querySelectorAll(
							'.fridge_list tbody .chk:checked');
					// 각 체크된 체크박스에서 품목 인덱스 추출하여 배열에 저장
					checkboxes.forEach(function(checkbox) {
						var row = checkbox.closest('tr'); // 체크박스의 부모 행(tr) 요소 가져오기
						var itemIndex = row.querySelector('.delete')
							.getAttribute('data-ingre-idx'); // 품목 인덱스 추출
